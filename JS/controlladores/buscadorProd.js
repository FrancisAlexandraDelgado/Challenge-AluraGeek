...
const mostrarResultados = async () => {
  try {
    ...
    data.forEach(({ imagenURL, name, price, categorie, description, id }) => {
      ...
      const categoriaLower = categorie.toLowerCase();
    ...
    if (cantidadResultados > 0) {
      const textoTitulo = `<h2 class="titulo">Resultados de la busqueda de "${textoBuscado}"</h2>`;
      tituloBusqueda.innerHTML = textoTitulo;
    }
    if (cantidadResultados === 0) {
      const textoTitulo = `<h2 class="titulo">No se encontraron resultados de "${textoBuscado}"</h2>`;
      tituloBusqueda.innerHTML = textoTitulo;
    }
  } catch (e) {
